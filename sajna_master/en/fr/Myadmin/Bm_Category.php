...
<script type="text/javascript">
	function addCat(){
		open("Bm_AddCat.php","ac","width=300,height=100,top=200,left=300");
	}
	function editCat(cid){
		open("Bm_EditCat.php?catid="+cid,"ac","width=300,height=100,top=200,left=300");
	}
</script>
<style type="text/css">
.head
...
<?php	
	$sel_cat = "SELECT * FROM bm_category ORDER BY Name";
...
<?php
	...
	while($rec = mysql_fetch_assoc($res_cat)){
		...
		echo "<tr>";
		echo "<td>$cnt</td>";
		echo "<td><a href='Bm_AddProd.php?cid=$cid' style='color:#000'>$rec[Name]</a></td>";
		echo "<td>";
		if($rec['cStatus']  == 0){
			echo "<a href='Bm_CatStatus.php?catid=$rec[Id]&status=1'>Activate</a>";
		}else{
			echo "<a href='Bm_CatStatus.php?catid=$rec[Id]&status=0'>Deactivate</a>";
		}
		echo "</td>";
		echo "<td align='center'><a href='javascript: void(0)' onclick='javascript: editCat($rec[Id])'>Edit</a> | <a href='Bm_deleteCatgory.php?catid=$rec[Id]'>Delete</a></td>";
		echo "</tr>";
		$cnt++;
	}
?>	 
